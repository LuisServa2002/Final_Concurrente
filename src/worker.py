...
def main():
    ...
    # parse peers: accept either multiple args or a single comma-separated string
    peers = []
    raw_peers = []
    for p in args.peers:
        if p is None:
        ...
        for part in str(p).split(','):
            part = part.strip()
            if part:
                raw_peers.append(part)
    for p in raw_peers:
        if ':' not in p:
            raise SystemExit(f"Invalid peer format: '{p}'. Expected host:port")
        h, po = p.split(':', 1)
